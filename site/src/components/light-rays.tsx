...
export default function Rays({
  ...
  useEffect(() => {
    ...
    // Parallex patch: keep the canvas sized to its container, skip frames
    // while off-screen, and stop re-rendering an unchanged still frame.
    let dirty = true;
    let visible = true;
    const resize = () => {
      ...
      material.uniforms.u_rayPos1.value = [
        (positionRef.current / 100) * w,
        RAY_Y_POSITION_1 * h,
      ];
      material.uniforms.u_rayPos2.value = [
        (positionRef.current / 100 + 0.02) * w,
        RAY_Y_POSITION_2 * h,
      ];
      dirty = true;
    };

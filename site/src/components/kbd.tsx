...
export function Kbd({ keys = [], className, active, listenToKeyboard = false }: KbdProps) {
  ...
  const getKeyDisplay = (item: KeyItem): string => {
    // Parallex patch: an explicit display label is shown as written.
    if (typeof item !== "string") return item.display;
    const key = item;
    const lowerKey = key.toLowerCase();
    // Parallex patch: only single-character keys are upper-cased; named keys
    // ("Space", "Return") stay in title case.
    return (
      keySymbolMap[lowerKey as keyof typeof keySymbolMap] ||
